/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { FaceRecognitionDataWhereInput } from "./FaceRecognitionDataWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { FaceRecognitionDataOrderByInput } from "./FaceRecognitionDataOrderByInput";

@ArgsType()
class FaceRecognitionDataFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => FaceRecognitionDataWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => FaceRecognitionDataWhereInput, { nullable: true })
  @Type(() => FaceRecognitionDataWhereInput)
  where?: FaceRecognitionDataWhereInput;

  @ApiProperty({
    required: false,
    type: [FaceRecognitionDataOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [FaceRecognitionDataOrderByInput], { nullable: true })
  @Type(() => FaceRecognitionDataOrderByInput)
  orderBy?: Array<FaceRecognitionDataOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { FaceRecognitionDataFindManyArgs as FaceRecognitionDataFindManyArgs };
